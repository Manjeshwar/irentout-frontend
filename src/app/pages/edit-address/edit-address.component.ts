...
export class EditAddressComponent implements OnInit, OnChanges {
  ...
  getUserDetails(){
    this.us.getUserDetailsByUid(this.userId).subscribe((dta) =>{
      this.allBillingAddress = JSON.parse(dta[0].billingaddress);
    });
  }
  ...
  updateAddress(){
    const controls = this.editAddressForm.controls;

    Object.keys(controls).forEach(key => {
      controls[key].markAsTouched();
    });

    const formVal = this.editAddressForm.value;
    this.allBillingAddress.forEach((res) => {
      if(res.id === this.addressToEdit.id){
        res.addresstype= formVal.addresstype;
        res.nickname= formVal.nickname;
        res.firstname= formVal.fname;
        res.lastname= formVal.lname;
        res.comapanyname= formVal.companyName;
        res.gst= formVal.gst;
        res.phone= formVal.mobile;
        res.addr= formVal.address;
        res.city= formVal.town;
        res.state= formVal.state;
        res.postal= formVal.pincode;
        res.default = res.default;

        if(this.editAddressForm.valid){
          this.us.addUpdateBillAddress(this.userId, JSON.stringify(this.allBillingAddress)).subscribe((addrs) => {
            this.addressChange.emit(this.getUserDetails());            
          });
        }
      }      
    });
  ...
  validatePostal() {
    const pincode = this.editAddressForm.value.pincode;
    if (pincode.length >= 6) {
